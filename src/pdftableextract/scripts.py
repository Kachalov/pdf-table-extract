...
from .core import output, process_page
...
def procargs():
    ...
    p.add_argument(
        ...
        help=(
            ...
            'Paints white outside this rectangle.'
        )
    )
    p.add_argument(
        '--line_length',
    ...
    p.add_argument(
        '-pad',
        help='initial image pading (pixels)',
        type=int,
        default=2
    )
    p.add_argument(
        ...
        action='append',
        help=(
            'paint white to the bitmap as left:top:right:bottom in length '
            'units. Done before painting black'
        )
    )
    p.add_argument(
        '-black',
        action='append',
        help=(
            'paint black to the bitmap as left:top:right:bottom in '
            'length units. Done after poainting white'
        )
    )
    p.add_argument(
        '-bitmap',
    ...
    p.add_argument(
        ...
        action='store_true',
        help=(
            'Stop after finding croping rectangle, and output debuging image '
            '(use -bitmap).'
        )
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        default='cells_xml',
        help=(
            "output type (table_chtml is colorized like '-checkcells') "
            "(default cells_xml)"
        )
    )
    p.add_argument(
        '--whitespace',
        '-w',
        choices=['none', 'normalize', 'raw'],
        default='normalize',
        help=(
            'What to do with whitespace in cells. none = remove it all, '
...
def main():
    try:
        ...
        if args.traceback:
            raise
        sys.exit('I/O Error running pdf-table-extract: {0}'.format(err))
    except OSError as err:
        print(
            'An OS Error occurred running pdf-table-extract: '
            'Is `pdftoppm` installed and available?'
        )
        if args.traceback:
            raise
        ...
        if args.traceback:
            raise
        sys.exit('Error while checking a subprocess call: {0}'.format(err))
