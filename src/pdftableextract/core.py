...
from .pnm import dumpImage, readPNM


def check_for_required_executable(name, command):
    """Checks for an executable called 'name' by running 'command' and
    supressing output. If the return code is non-zero or an OS error occurs,
    ...
    try:
        ...
        message = 'Error running {0}. Command failed: {1} {2}'.format(
            name,
            " ".join(command),
            e
        )
...
def popen(name, command, *args, **kwargs):
    try:
        ...
        message = (
            'Error running {0}. Is it installed correctly? Error: {1}'
        ).format(name, e)
        raise OSError(message)
    except Exception as e:
        raise
...
def process_page(
        infile, pgs, outfilename=None, greyscale_threshold=25, page=None,
        crop=None, line_length=0.17, bitmap_resolution=300, name=None, pad=2,
        white=None, black=None, bitmap=False, checkcrop=False,
        checklines=False, checkdivs=False, checkcells=False,
    ...
    bmp = np.ones((height, width), dtype=bool)
    bmp[pad:height-pad, pad:width - pad] = (
        data[:, :] > int(255.0 * greyscale_threshold / 100.0)
    )
    ...
    # Find bounding box.
    t = 0
    while t < height and np.sum(bmp[t, :] == 0) == 0:
        t = t + 1
    ...
    b = height - 1
    while b > t and np.sum(bmp[b, :] == 0) == 0:
        b = b-1
    ...
    l = 0
    while l < width and np.sum(bmp[:, l] == 0) == 0:
        l = l + 1
    ...
    r = width - 1
    while r > l and np.sum(bmp[:, r] == 0) == 0:
        r = r - 1
    if r < width - 1:
        r = r + 1
    ...
    if black:
        for b in black:
            (l, t, r, b) = [
                bitmap_resolution * float(x) + pad for x in b.split(":")
            ]
            bmp[t:b + 1, l:r + 1] = 0
    ...
    vs = np.zeros(width, dtype=int)
    for i in range(width):
        dd = np.diff(np.where(bmp[:, i])[0])
        if len(dd) > 0:
            v = max(dd)
            if v > lthresh:
                vs[i] = 1
            ...
            if bmp[0, i] == 0:
                vs[i] = 1
    vd = (np.where(np.diff(vs[:]))[0] + 1)

    hs = np.zeros(height, dtype=int)
    for j in range(height):
        dd = np.diff(np.where(bmp[j, :] == 1)[0])
        if len(dd) > 0:
            h = max(dd)
            if h > lthresh:
                hs[j] = 1
    ...
    def isDiv(a, l, r, t, b):
        ...
        return np.sum(np.sum(bmp[t:b, l:r], axis=a) == 0) > 0
    ...
    cells = []
    touched = np.zeros((len(hd), len(vd)), dtype=bool)
    j = 0
    while j * 2 + 2 < len(hd):
        i = 0
        while i * 2 + 2 < len(vd):
    ...
    def getCell((i, j, u, v)):
        ...
        p = popen(
            "pdftotext",
            (
                "pdftotext -r %d -x %d -y %d -W %d -H %d -layout -nopgbrk "
                "-f %d -l %d %s -"
            ) % (
                bitmap_resolution,
                l - pad,
        ...
        if whitespace != 'raw':
            ...
            if len(ret) > 0:
                ret = ret[(1 if ret[0] == ' ' else 0):len(
                    ret) - (1 if ret[-1] == ' ' else 0)]
        return (i, j, u, v, pg, ret)

    if boxes:
        cells = [
            x + (pg, "",) for x in cells if (
...
def output(
        cells, pgs, cells_csv_filename=None, cells_json_filename=None,
        cells_xml_filename=None, table_csv_filename=None,
        table_html_filename=None, table_list_filename=None, infile=None,
        name=None, output_type=None):
...
def o_cells_csv(
        cells, pgs, outfile=None, name=None, infile=None, output_type=None):
    outfile = outfile or sys.stdout
    csv.writer(outfile, dialect='excel').writerows(cells)


def o_cells_json(
        cells, pgs, outfile=None, infile=None, name=None, output_type=None):
...
def o_table_html(
        cells, pgs, outfile=None, output_type=None, name=None, infile=None):
    ...
    for k in range(nc):
        ...
        if output_type == "table_chtml":
            td.setAttribute(
                "style",
                "background-color: #%02x%02x%02x" % tuple(
                    128 + col(k / (nc + 0.))
                )
            )
        tr.appendChild(td)
